...
class Homework(Trie):
    ...
    def count_words_with_suffix(self, pattern) -> int:
        ...
        return self._recursive_suffix_search(current, "", pattern)

    def _recursive_suffix_search(self, node, current_word, pattern):
        count = 0
        # Check if the word formed so far ends with the pattern
        if node.value is not None and current_word.endswith(pattern):
            count = 1
        
        for char, child_node in node.children.items():
            count += self._recursive_suffix_search(child_node, current_word + char, pattern)
            
        return count
    
    def has_prefix(self, prefix) -> bool:
        if not isinstance(prefix, str) or not prefix:
            raise TypeError(f"Illegal argument for hasPrefix: prefix = {prefix} must be a non-empty string")
...
if __name__ == "__main__":
    ...
    # Check amount of words ending with a given suffix
